...
const LineSidebar: React.FC<LineSidebarProps> = ({
  ...
  // Choose external controlled state if provided, otherwise internal state
  const activeIndex = activeIndexProp !== null ? activeIndexProp : internalActiveIndex;
  ...
  const handlePointerLeave = useCallback(() => {
    const itemElements = itemRefs.current;
    targetsRef.current = itemElements.map(() => 0);
    startLoop();
  }, [startLoop]);
      ...
      >
        {items.map((label, index) => (
          ...
          >
            ...
            <span className="relative inline-flex items-baseline leading-[1.2] [color:color-mix(in_srgb,var(--accent-color)_calc(var(--effect,0)*100%),var(--text-color))] [font-size:var(--font-size)] [transform:translateX(calc(var(--effect,0)*var(--max-shift)))] font-bold transition-transform">
              {showIndex && (
